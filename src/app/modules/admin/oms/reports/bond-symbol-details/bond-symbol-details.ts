...
@Component({
  selector: 'bond-symbol-details',
  templateUrl: './bond-symbol-details.html',

  encapsulation: ViewEncapsulation.None,
})

export class BondSymbolDetails {
  ...
  @ViewChild('tenure',{ static: false }) tenure: wjcInput.InputMask;
  ...
  getExchangeMarketSecuritiesList(exchangeId , marketId) {
    this.listingService.getExchangeMarketSecuritiesList(exchangeId, marketId).subscribe(
        data => {
            this.splash.hide();
            if (data == null){  return; }
            this.symbols = data;
  ...
  updateSecurityDetails(data) {
    ...
    this.tenure.value = this.securityMarketDetails.tenure.toString();
