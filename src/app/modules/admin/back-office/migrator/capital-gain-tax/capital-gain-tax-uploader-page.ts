...
@Component({
  selector: 'capital-gain-tax-uploader-page',
  templateUrl: './capital-gain-tax-uploader-page.html',
})

export class CapitalGaintaxUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  public oClientHolding: Migrator;
  ...
  constructor(private changeDetectorRef: ChangeDetectorRef, private listingService: ListingService,
    ...
    this.oClientHolding = new Migrator();
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    this.oClientHolding.participantId = AppConstants.participantId;
    this.oClientHolding.requestFileBase64 = this.file_srcs[0];
    AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oClientHolding));
    this.listingService.uploadCapitalGainTax(this.oClientHolding).subscribe(
      data => {
  ...
  public onDownloadDocumentAction(filecontent_: string) {
    ...
    let fileName = 'CapitalGaintax.txt';
