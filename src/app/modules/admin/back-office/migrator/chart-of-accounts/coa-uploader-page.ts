...
@Component({
  selector: 'coa-uploader-page',
  templateUrl: './coa-uploader-page.html',
})

export class COAUploaderPage implements OnInit {
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    if (this.loaderType) {
      this.COAControlAccount();
    } else {
      this.COADetailAccount();
    }

    // }
  }

  UpdateDetailRecords(oCliHol_: Migrator) {
    ...
    if (this.error_records > 0) {
      this.onDownloadDetailDocumentAction(oCliHol_.responseFileBase64);
    }
  }
  UpdateControlRecords(oCliHol_: Migrator) {
    AppUtility.printConsole('in UpdateRecords');
    this.error_records = oCliHol_.errorRecords;
    this.loaded_records = oCliHol_.loadedRecords;
    this.dialogCmp.statusMsg = oCliHol_.serverResponse;
    this.dialogCmp.showAlartDialog('Notification');
    if (this.error_records > 0) {
      this.onDownloadControlDocumentAction(oCliHol_.responseFileBase64);
    }
  }

  public onDownloadDetailDocumentAction(filecontent_: string) {
    AppUtility.printConsole('file: ' + filecontent_);
    let base64Data = filecontent_;
    let contentType = 'application/vnd.ms-excel';
    let fileName = 'COA_Detail_Account_Error.txt';
    downloadAPI(base64Data, fileName, contentType);
  }
  public onDownloadControlDocumentAction(filecontent_: string) {
    AppUtility.printConsole('file: ' + filecontent_);
    let base64Data = filecontent_;
    let contentType = 'application/vnd.ms-excel';
    let fileName = 'COA_Control_Account_Error.txt';
    downloadAPI(base64Data, fileName, contentType);
  }
  ...
  public COADetailAccount() {
    this.listingService.uploadDetailAccount(this.oClients).subscribe(
      data => {
        this.loader.hide();
        AppUtility.printConsole('received data: ' + JSON.stringify(data));
        this.UpdateDetailRecords(data);
        this.file_srcs = [];
        if (!AppUtility.isEmpty(this.fileInput_)) {
          this.fileInput_.value = '';
        }
  ...
  public COAControlAccount() {
    this.listingService.uploadControlAccount(this.oClients).subscribe(
      data => {
        this.loader.hide();
        AppUtility.printConsole('received data: ' + JSON.stringify(data));
        this.UpdateControlRecords(data);
        this.file_srcs = [];
        if (!AppUtility.isEmpty(this.fileInput_)) {
          this.fileInput_.value = '';
        }
