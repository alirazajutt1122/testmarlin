...
@Component({
  selector: 'commission-slab-uploader-page',
  templateUrl: './commission-slab-uploader-page.html',
})

export class CommissionSlabUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  public oCommissionSlab: Migrator;
  ...
  public loaderType: boolean = true;
  ...
  constructor(private appState: AppState, private changeDetectorRef: ChangeDetectorRef, private listingService: ListingService, private _fb: FormBuilder, public userService: AuthService2, private translate: TranslateService,private loader : FuseLoaderScreenService) {
    ...
    this.oCommissionSlab = new Migrator();
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    this.oCommissionSlab.participantId = AppConstants.participantId;
    this.oCommissionSlab.requestFileBase64 = this.file_srcs[0];
    AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oCommissionSlab));
    this.appState.showLoader = true;
    if (this.loaderType) {
      this.CommissionSlabRequest();
    } else {
      this.AssignmentOfSlabsRequest();
    }

    // }
  }

  UpdateCommissionRecords(oCliHol_: Migrator) {
    AppUtility.printConsole('in UpdateRecords');
    this.error_records = oCliHol_.errorRecords;
    this.loaded_records = oCliHol_.loadedRecords;
    this.dialogCmp.statusMsg = oCliHol_.serverResponse;
    this.dialogCmp.showAlartDialog('Notification');
    if (this.error_records > 0) {
      this.onDownloadCommissionSlabAction(oCliHol_.responseFileBase64);
    }
  }
  UpdateAssignmentRecords(oCliHol_: Migrator) {
    AppUtility.printConsole('in UpdateRecords');
    this.error_records = oCliHol_.errorRecords;
    this.loaded_records = oCliHol_.loadedRecords;
    this.dialogCmp.statusMsg = oCliHol_.serverResponse;
    this.dialogCmp.showAlartDialog('Notification');
    if (this.error_records > 0) {
      this.onDownloadAssignemntSlabAction(oCliHol_.responseFileBase64);
    }
  }

  public onDownloadCommissionSlabAction(filecontent_: string) {
    AppUtility.printConsole('file: ' + filecontent_);
    let base64Data = filecontent_;
    let contentType = 'application/vnd.ms-excel';
    let fileName = 'Commission_slab_error.csv';
    downloadAPI(base64Data, fileName, contentType);
  }

  public onDownloadAssignemntSlabAction(filecontent_: string) {
    AppUtility.printConsole('file: ' + filecontent_);
    let base64Data = filecontent_;
    let contentType = 'application/vnd.ms-excel';
    let fileName = 'Assignment_of_Slabs_Error.csv';
    downloadAPI(base64Data, fileName, contentType);
  }
  ...
  public onLoaderChangeEvent(flag: boolean) {
    this.loaderType = flag;
  }

  public CommissionSlabRequest() {
    this.listingService.uploadCommissionSlab(this.oCommissionSlab).subscribe(
      data => {
        this.appState.showLoader = false;

        AppUtility.printConsole('received data: ' + JSON.stringify(data));
        this.UpdateCommissionRecords(data);
        this.file_srcs = [];
        if (!AppUtility.isEmpty(this.fileInput_)) {
          this.fileInput_.value = '';
        }
  ...
  public AssignmentOfSlabsRequest() {
    this.listingService.uploadAssignmentOfSlab(this.oCommissionSlab).subscribe(
      data => {
        this.appState.showLoader = false;

        AppUtility.printConsole('received data: ' + JSON.stringify(data));
        this.UpdateAssignmentRecords(data);
        this.file_srcs = [];
        if (!AppUtility.isEmpty(this.fileInput_)) {
          this.fileInput_.value = '';
        }
