...
@Component({
  selector: 'vault-uploader-page',
  templateUrl: './vault-uploader-page.html',
})

export class VaultUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  public oVault: Migrator;
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      this.oVault.participantId = AppConstants.participantId;
      this.oVault.requestFileBase64 = this.file_srcs[0];
      this.loader.show();

      AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oVault));
      this.listingService.uploadVaults(this.oVault).subscribe(
        data => {
          this.loader.hide();
  ...
  public onDownloadDocumentAction(filecontent_: string) {
    ...
    let fileName = 'Vault_Loader_Error.txt';
