...
@Component({
  selector: 'clients-uploader-page',
  templateUrl: './clients-uploader-page.html',
})

export class ClientsUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  exchangeList: any[] = [];
  exchangeSelected: number = 0;
  public oClients: Migrator;
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    this.oClients.participantId = AppConstants.participantId;
    this.oClients.requestFileBase64 = this.file_srcs[0];
    this.oClients.exchange = this.exchangeSelected;
    this.loader.show();
    AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oClients));
    if (this.loaderType) {
      this.clientHolding();
    } else {
      this.clientBalance();
  ...
  UpdateRecords(oCliHol_: Migrator) {
    ...
    if (this.error_records > 0) {
      this.onDownloadDocumentAction(oCliHol_.responseFileBase64);
    }
  }
  UpdateHoldingRecords(oCliHol_: Migrator) {
    AppUtility.printConsole('in UpdateRecords');
    this.error_records = oCliHol_.errorRecords;
    this.loaded_records = oCliHol_.loadedRecords;
    this.dialogCmp.statusMsg = oCliHol_.serverResponse;
    this.dialogCmp.showAlartDialog('Notification');
    if (this.error_records > 0) {
      this.onDownloadHoldingDocumentAction(oCliHol_.responseFileBase64);
    }
  }
  ...
  public onDownloadDocumentAction(filecontent_: string) {
    ...
    let fileName = 'Clients_error.csv';
    downloadAPI(base64Data, fileName, contentType);
  }
  public onDownloadHoldingDocumentAction(filecontent_: string) {
    AppUtility.printConsole('file: ' + filecontent_);
    let base64Data = filecontent_;
    let contentType = 'application/vnd.ms-excel';
    let fileName = 'Holding_error.csv';
    downloadAPI(base64Data, fileName, contentType);
  }
    ...
    this.listingService.getParticipantExchangeList(AppConstants.participantId).subscribe(
      data => {
        ...
        if (data != null) {
          ...
          this.exchangeSelected = (AppConstants.exchangeId === null) ? this.exchangeList[0].exchangeId : AppConstants.exchangeId;
  ...
  public clientBalance() {
    this.listingService.uploadClients(this.oClients).subscribe(
      data => {
        this.loader.hide();
  ...
  public clientHolding() {
    this.listingService.uploadHolding(this.oClients).subscribe(
      data => {
        this.loader.hide();
        AppUtility.printConsole('received data: ' + JSON.stringify(data));
        this.UpdateHoldingRecords(data);
        this.file_srcs = [];
        if (!AppUtility.isEmpty(this.fileInput_)) {
          this.fileInput_.value = '';
        }
  ...
  public onExchangeChange(val: number) {
    this.exchangeSelected = val;
  }
}
