...
@Component({
  selector: 'clients-setup-uploader-page',
  templateUrl: './clients-setup-uploader-page.html',
})

export class ClientsSetupUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  public oClientSetup: Migrator;
  ...
  constructor(private appState: AppState, private changeDetectorRef: ChangeDetectorRef, private listingService: ListingService, private _fb: FormBuilder, public userService: AuthService2, private translate: TranslateService,private loader : FuseLoaderScreenService) {
    ...
    this.oClientSetup = new Migrator();
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    this.oClientSetup.participantId = AppConstants.participantId;
    this.oClientSetup.requestFileBase64 = this.file_srcs[0];
    this.loader.show();

    AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oClientSetup));
    this.listingService.uploadClientsSetup(this.oClientSetup).subscribe(
      data => {
  ...
  public onDownloadDocumentAction(filecontent_: string) {
    ...
    let fileName = 'ClientsSetup_error.txt';
