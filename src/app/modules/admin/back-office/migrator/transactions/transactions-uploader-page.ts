...
@Component({
  selector: 'transactions-uploader-page',
  templateUrl: './transactions-uploader-page.html',
})

export class TransactionsUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  public oTransactions: Migrator;
  public debit: boolean = true;
  public loaded_records: number = 0;
  public openTradeRecords: number = 0;
  public fullReleaseRecords: number = 0;
  public multiReleaseRecords: number = 0;
  public partialReleaseRecords: number = 0;
  public rollOverRecords: number = 0;
  ...
  constructor(private appState: AppState, private changeDetectorRef: ChangeDetectorRef, private listingService: ListingService, private _fb: FormBuilder, public userService: AuthService2, private translate: TranslateService,private loader : FuseLoaderScreenService) {
    ...
    this.oTransactions = new Migrator();
    ...
    this.transactionTypeList = [
      ...
      {
        'value': 'MFSD - MFN - Initiator',
        'abbreviation': 'MFSD'
      },
      {
        'value': 'MFSN - MFR - Release',
        'abbreviation': 'MFSN'
      },
      {
        'value': 'REG/Future',
  ...
  public clearFields() {
    ...
    this.openTradeRecords = 0;
    this.fullReleaseRecords = 0;
    this.multiReleaseRecords = 0;
    this.partialReleaseRecords = 0;
    this.rollOverRecords = 0;
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      if (model.transactionType == "MTSN" || model.transactionType == "MFSN")
        this.isVisible = true;
      else
        this.isVisible = false;

      this.oTransactions.participantId = AppConstants.participantId;
      this.oTransactions.transactionType = model.transactionType;
      this.oTransactions.requestFileBase64 = this.file_srcs[0];
      this.loader.show();

      AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oTransactions));
      this.listingService.uploadTransactions(this.oTransactions).subscribe(
        data => {
          this.loader.hide();
  ...
  UpdateRecords(oTrans_: Migrator) {
    ...
    this.openTradeRecords = oTrans_.openTradeRecords;
    this.fullReleaseRecords = oTrans_.fullReleaseRecords;
    this.multiReleaseRecords = oTrans_.multiReleaseRecords;
    this.partialReleaseRecords = oTrans_.partialReleaseRecords;
    this.rollOverRecords = oTrans_.rollOverRecords;
  ...
  public onDownloadDocumentAction(filecontent_: string) {
    ...
    let fileName = 'Transactions_Loader_Error.txt';
