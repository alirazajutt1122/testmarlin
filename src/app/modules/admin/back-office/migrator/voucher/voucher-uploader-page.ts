...
@Component({
  selector: 'voucher-uploader-page',
  templateUrl: './voucher-uploader-page.html',
})

export class VoucherUploaderPage implements OnInit {
  public myForm: FormGroup;
  errorMessage: string;
  //claims: any;
  public oVoucher: Migrator;
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    if (isValid) {
      ...
      this.loader.show();
      this.oVoucher.participantId = AppConstants.participantId;
      this.oVoucher.requestFileBase64 = this.file_srcs[0];
      AppUtility.printConsole('btnUpload called: ' + JSON.stringify(this.oVoucher));
      this.listingService.uploadVouchers(this.oVoucher).subscribe(
        data => {
          this.loader.hide();
  ...
  public onDownloadDocumentAction(filecontent_: string) {
    ...
    let fileName = 'Voucher_Loader_Error.lis';
