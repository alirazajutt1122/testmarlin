...
export class CdcReconcilation implements OnInit {
  ...
  public btnUpload(model: any, isValid: boolean) {
    ...
    this.listingService.uploadCDCReconcilationNew(this.oCDCMigrator).subscribe(
      data => {
        this.loader.hide();
  ...
  exportExcel() {
    wjcGridXlsx.FlexGridXlsxConverter.save(this.flexDetail, { includeColumnHeaders: this.includeColumnHeader, includeCellStyles: false }, 'CDC Reconcilation.xlsx');
}
...
exportPDF() {
    gridPdf.FlexGridPdfConverter.export(this.flexDetail, 'CDC Reconcilation' + new Date().toLocaleString() + '.pdf', {
        maxPages: 10,
