...
@Component({
    selector : 'redemption-payment-rpt',
    templateUrl : './redemption-payment-rpt.html'
})
export class RedemptionPaymentRpt implements OnInit{
    ...
    onSecurityChange(security){
        // if(AppUtility.isValidVariable(Secur))
    ...
    getRedemptionPaymentDetail(value,isValid){
        this.redemptionPaymentList = [];
        ...
        if(isValid){
           ...
           this.loader.show()
           this.listingService.getRedemptionPaymentList(couponPayload).subscribe((resData) => {
              this.loader.hide();
              if(!AppUtility.isEmptyArray(resData)){
                 this.filterRedemptionPaymentList(resData,value);
              } else{
    ...
    filterRedemptionPaymentList(data,selectedValues){
     
       if(AppUtility.isValidVariable(data)){
           if(selectedValues.security === AppConstants.ALL_VAL && selectedValues.clientCode === AppConstants.ALL_VAL){
              this.redemptionPaymentList = data;
              return;
           }
           
           this.redemptionPaymentList = data.filter((coupon) => {
               return this.symbol === coupon.symbol || selectedValues.clientCode === coupon.accountCode;
           })
           
           if(AppUtility.isEmptyArray(this.redemptionPaymentList)){
              this.dialogCmp.statusMsg = AppConstants.MSG_NO_DATA_FOUND;
              this.dialogCmp.showAlartDialog('Error');
              return;
           }
    ...
    exportExcel() {
        wjcGridXlsx.FlexGridXlsxConverter.save(this.flexGrid, { includeColumnHeaders: this.includeColumnHeader, includeCellStyles: false }, 'Redemption Payment.xlsx');
    }
    ...
    exportPDF() {
        gridPdf.FlexGridPdfConverter.export(this.flexGrid, 'Redemption Payment' + new Date().toLocaleString() + '.pdf', {
            maxPages: 10,
