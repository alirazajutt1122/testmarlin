...
@Component({
    selector: 'client-last-activity-rpt',
    templateUrl: './client-last-activity-rpt.html',
    encapsulation: ViewEncapsulation.None,
})
export class ClientLastActivityRpt implements OnInit {
    ...
    pdfSrc: String
    pdf = false
    fileNameForDownload = "ClientLastActivityReport.pdf"
   ...
   *********************************/
    ...
    public printReport(model: any, isValid: boolean) {
        ...
        if (isValid) {
            ...
            if (this.email) {
                this.clientLastActivityWithEmail(exchange, security, branch, startDate, endDate, agent);
            } else {
                this.clientLastActivityWithoutEmail(exchange, security, branch, startDate, endDate, agent);
            }
        }
    }
    ...
    public clientLastActivityWithoutEmail(exchange, security, branch, startDate, endDate, agent) {
        this.reportService.getClientLastActivity(this.params.PARTICIPANT_ID, startDate, endDate, this.params.EXCHANGE_ID,
            exchange, this.params.PARTICIPANT_BRANCH_ID, branch, encodeURIComponent(this.params.FROM_ACCOUNT.valueOf()),
            encodeURIComponent(this.params.TO_ACCOUNT.valueOf()), this.params.SECURITY_ID, security, this.params.AGENT_ID, agent, this.mail, this.lang)
            .subscribe(
                restData => {
                    this.loader.hide();
                    var reportData: any;
    ...
    public clientLastActivityWithEmail(exchange, security, branch, startDate, endDate, agent) {
        this.reportService.getClientLastActivityWithEmail(this.params.PARTICIPANT_ID, startDate, endDate, this.params.EXCHANGE_ID,
            exchange, this.params.PARTICIPANT_BRANCH_ID, branch, encodeURIComponent(this.params.FROM_ACCOUNT.valueOf()),
            encodeURIComponent(this.params.TO_ACCOUNT.valueOf()), this.params.SECURITY_ID, security, this.params.AGENT_ID, agent)
            .subscribe(
                restData => {
                    this.loader.hide();
                    var reportData: any;
                    ...
                    // var w = window.open(reportData.reportBase64/*,"_self"*/);
                    // downloadAPI(reportData.reportBase64, reportData.reportName, reportData.contentType);
                    this.UpdateRecords(reportData, "Mailed_Person");
                },
                error => {
                    this.loader.hide();
                    this.dialogCmp.statusMsg = error.message;
